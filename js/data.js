const laptopData = {
    ...
    <!-- SCRIPT FOR COMPARISON PAGE -->
    <script id="comparison-page-script">
        document.addEventListener('DOMContentLoaded', () => {
            const comparisonGrid = document.getElementById('comparison-grid');
            const comparisonMessage = document.getElementById('comparison-message');

            if (typeof ChartManager !== 'undefined') {
                ChartManager.clearChartInstances(); 
            }

            const selectedLaptopIdsJSON = localStorage.getItem('selectedLaptopsForComparison');
            let selectedLaptopIds = [];
            if (selectedLaptopIdsJSON) {
                try {
                    selectedLaptopIds = JSON.parse(selectedLaptopIdsJSON);
                } catch (e) {
            ...
            if (!Array.isArray(selectedLaptopIds) || selectedLaptopIds.length < 2) {
                if(comparisonMessage) {
                    comparisonMessage.textContent = 'Please select at least two laptops from the Explorer page to compare.';
            ...
            if(comparisonMessage) comparisonMessage.classList.add('hidden');
            if(comparisonGrid) comparisonGrid.innerHTML = ''; 

            const laptopsToCompare = selectedLaptopIds.slice(0, 3); 

            laptopsToCompare.forEach(id => {
                const laptop = typeof laptopData !== 'undefined' ? laptopData[id] : null; 
                if (laptop) {
                    const card = document.createElement('div');
                    card.className = 'comparison-card card-shadow flex flex-col';
                    
                    let dgpuText = laptop.details.dgpu && laptop.details.dgpu !== "None" ? `<li><strong>dGPU:</strong> ${laptop.details.dgpu}</li>` : '';

                    card.innerHTML = `
                        <h3 class="text-xl font-bold mb-3 text-primary dark:text-blue-400">${laptop.name}</h3>
                        <img src="${laptop.image}" alt="${laptop.name}" class="w-full h-40 object-cover rounded-md mb-3" onerror="this.onerror=null;this.src='https.placehold.co/400x200/212529/F8F9FA?text=No+Image';">
                        <p class="text-sm text-medium mb-1"><strong>Score:</strong> ${laptop.score}%</p>
                        ${laptop.details.cpuBenchmark ? `<p class="text-sm text-medium mb-3"><strong>CPU Bench:</strong> ~${laptop.details.cpuBenchmark.score} <span class="text-xs">(${laptop.details.cpuBenchmark.source})</span></p>` : ''}
                        
                        <h4 class="font-semibold text-dark dark:text-gray-200 text-sm mt-2 mb-1">Key Specs:</h4>
                        <ul class="text-xs text-medium dark:text-gray-300 list-none space-y-0.5 mb-3">
                            <li><strong>CPU:</strong> ${laptop.details.cpu}</li>
                            <li><strong>iGPU:</strong> ${laptop.details.igpu}</li>
                            ${dgpuText}
                            <li><strong>RAM:</strong> ${laptop.details.ram}</li>
                            <li><strong>Storage:</strong> ${laptop.details.storage}</li>
                            <li><strong>macOS:</strong> ${laptop.details.macosSupport.min} - ${laptop.details.macosSupport.max}</li>
                        </ul>
                        
                        <div class="comparison-chart-container mt-auto">
                            <canvas id="compare-chart-${id}"></canvas>
                        </div>
                        <a href="laptop_detail.html?id=${id}" class="mt-4 text-center text-primary dark:text-blue-400 hover:underline text-sm">View Full Details &rarr;</a>
                    `;
                    if(comparisonGrid) comparisonGrid.appendChild(card);

                    const chartCtx = document.getElementById(`compare-chart-${id}`).getContext('2d');
                    const currentTheme = document.documentElement.classList.contains('dark') ? 'dark' : 'light';
                    if (typeof ChartManager !== 'undefined') {
                         ChartManager.createRadarChart(chartCtx, laptop.name, laptop.details.chartData, currentTheme, true); 
                    }
                } else {
                    console.warn(`Laptop data for ID ${id} not found on comparison page.`);
                     const errorCard = document.createElement('div');
                     errorCard.className = 'comparison-card card-shadow flex flex-col items-center justify-center';
                     errorCard.innerHTML = `<p class="text-red-500">Error: Data for laptop ID "${id}" not found.</p>`;
                     if(comparisonGrid) comparisonGrid.appendChild(errorCard);
                }
            });
            ...
            if (findBarHeader) {
                findBarHeader.addEventListener('keypress', function(event) {
                    if (event.key === 'Enter') {
                        ...
                        if (searchTerm) {
                            window.location.href = `explorer.html?search=${encodeURIComponent(searchTerm)}`;
                        } else {
